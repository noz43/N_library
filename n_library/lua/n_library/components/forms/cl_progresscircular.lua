...
function PANEL:Paint(w, h)
    local cx = w / 2
    local cy = h / 2
    local radius = math.min(w, h) / 2 - 2
    
    if self.Title and self.Title ~= "" then
        draw.SimpleText(self.Title, "N.Font.12.Bold", cx, cy - radius - 12, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    ...
    if self.ShowLabel then
        ...
        draw.SimpleText(label, "N.Font.14.Bold", cx, cy, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
    
    return true
end

vgui.Register("N.ProgressCircular", PANEL, "DPanel")